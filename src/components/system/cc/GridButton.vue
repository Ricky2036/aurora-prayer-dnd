...
const hasBadge = computed(() => !!props.item?.hasBadge)
...
/** 展开态响铃容器不激活底色；有底板胶囊（如蓝牙）容器不激活；无底板胶囊（如 Dark Theme）激活时整体着色 */
...
const badgeStyle = computed(() => {
  ...
  // 默认不加圆形底板（如 Dark Theme 等），保持纯净图标直接置于胶囊上
  if (!hasBadge.value) {
    return {
  ...
  // 显式配置 hasBadge 的 2x1 图标（如 Bluetooth）
  if (props.editing) {
    return {
